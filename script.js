define(['jquery', 'underscore', 'twigjs'], function ($, _, Twig) {
    var CustomWidget = function () {
        ...
        this.callbacks = {
            ...
            // Добавьте обработчик для кнопки "Получить информацию о товарах"
            render: function () {
                var self = this;

                // Обработчик для кнопки
                $(document).on('click', '.' + self.params.widget_code + '__get-products-button', function () {
                    // Вызываем ваш backend скрипт, передавая в запрос параметры сделки
                    self.crm_post(
                        'https://your-backend-url.com/widget-backend.php',
                        {
                            deal_id: self.params.card_id
                        },
                        function (response) {
                            if (response && response.length > 0) {
                                // Построение модального окна с информацией о товарах
                                var productsModalContent = '<h2>Информация о товарах:</h2><ul>';
                                for (var i = 0; i < response.length; i++) {
                                    productsModalContent += '<li>' + response[i].name + ': ' + response[i].quantity + '</li>';
                                }
                                productsModalContent += '</ul>';

                                AMOCRM.notifications.show_message({
                                    title: 'Информация о товарах',
                                    text: productsModalContent,
                                    date: Date.now(),
                                    icon: 'icon-info',
                                    auto_close: true
                                });
                                ...
                                AMOCRM.notifications.show_message({
                                    title: 'Информация о товарах',
                                    text: 'Для этой сделки нет товаров',
                                    date: Date.now(),
                                    icon: 'icon-info',
                                    auto_close: true
                                });
